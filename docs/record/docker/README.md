...
- COPY ：拷贝宿主机本地文件倒镜像的指定目录。语法：COPY 宿主机路径 目标路径(镜像内路径) 例如：COPY \/usr\/local\/bin\/
  \/usr\/local\/bin\/
- RUN ：运行命令，执行linux的shell命令，一般是安装过程的命令。语法：RUN 命令 例如：RUN apt-get update && apt-get install -y
  curl
...
- 创建nginx容器，参考 docker 安装 nginx及配置(
  要将容器加入到与后端服务容器统一网络中，在同一网络中nginx配置代理就不需要写ip地址了)
...
Docker Compose 通过一个单独的 docker-compose.yml 模板文件（YAML 格式）来定义一组相关联的应用容器，帮助我们实现多个相互关联的Docker容器快速部署。
...
## Dockerfile 和 DockerCompose 区别

- Dockerfile
- 定义
    - Dockerfile 是一个文本文件，包含一系列指令，用于构建 Docker 镜像
    - 它定义了如何从基础镜像开始，逐步构建出最终的镜像
- 特点
    - 单服务导向：通常用于构建单个容器镜像
    - 构建时配置：在镜像构建阶段执行指令
    - 指令驱动：使用特定的 Docker 指令（FROM、RUN、COPY、CMD 等）

- Docker Compose
    - 定义
        - Docker Compose 是一个工具，用于定义和运行多容器的 Docker 应用程序
        - 通过 docker-compose.yml 文件配置多个服务的协调运行
    - 特点
        - 多服务编排：可以同时管理多个容器服务
        - 运行时配置：配置容器运行参数、网络、卷等
        - 声明式语法：使用 YAML 格式描述整个应用栈

使用关系：

- Dockerfile → 构建镜像 → Docker Compose → 编排运行
- Dockerfile 用于创建镜像
- Docker Compose 用于运行和管理基于这些镜像的容器
