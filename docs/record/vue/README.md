...
[参考@vensst/js-toolkit 中 initDataView 方法](/js-toolkit/api.html#initdataview)
...
### 使用 rem （lib-flexible + postcss-px2rem）

lib-flexible 会根据屏幕宽度动态改变 html 的 font-size 值，通过 postcss-px2rem 将 px 转换为 rem，从而实现 rem 布局。
...
3. 新建 flexible.js 文件，复制以下代码，在 main.js 中引入。  
   为什么要新建？因为原来的 lib-flexible，内部 refreshRem 方法内部大于宽度为 540px 固定以 540px 不做适配了。
...
- 设计稿宽度为 1920px，高度为 1080px；
- 100vw=视口宽度，100vh=视口高度；
- 电脑为 16:9 标准屏幕下浏览器 f11 全屏的时候，视口宽度和高度就是 1920px 和 1080px，也就是 100vw=视口宽度=1920px，100vh=视口宽度=1080px；
- 假设 A 块区域在设计稿宽度 300px、高度 200px，那么在浏览器它的 vw 宽度就是 300px/1920px
  *100vw=15.625vw，vh 高度就是 200px/1080px*100vh=18.518vh；屏幕放大缩小，A 块区域的宽高也会跟着变化，这样就实现了自适应布局。
...
**报错原因：**  
在 vue 项目 template 中使用了可选链运算符（?.）,vue2.6 不支持可选链运算符（?.）为什么 script 中可以使用，template 中不可以使用呢？
因为 template 中使用的是 vue 的模板语法，而 vue2.6 不支持可选链运算符（?.）,所以会报错。
...
3. lodash 的 get 方法代替
4. 升级 vue 到 2.7+版本
5. 如果是低版本的 vue，可以使用 babel 插件来解决（针对 js 部分)
   ，template 模板部分可使用[vue-template-babel-compiler（不是官方的）](https://github.com/JuniorTour/vue-template-babel-compiler#readme)
   ```shell
...
1. vue3.0+支持可选链运算符（?.）和空值合并运算符（??），vue2 中建议 template 中不使用高阶运算符语法硬要使用的话用 2、3 方式解决，js 中可以使用 babel 插件解决。
...
- **ESM**（ES 模块，或者 ES6 Module） 2015 年 ES6（ECMAScript 6.0 简称 ES6（ECMAScript 是制定 JavaScript 语言的标准组织））
  提出的模块化。
  - 一个 js 脚本就是一个模块，模块之间可以相互加载，通过 export 和 import 来交换功能，让一个模块可以使用另一个模块的函数或变量。
  - ESM（服务器和浏览器）取代 CommonJS（如服务器 node）和 AMD（浏览器），现代浏览器通过<script type="module"></script>
    使用，在之前的浏览器不兼容所以使用垫片 Polyfill 来处理兼容性问题。

:::
[vite](https://cn.vitejs.dev/guide/why.html) 所要解决的问题是开发大型项目，预构建开发服务器慢，模块热替换（HMR）慢，影响开发效率。

- 开发环境编译使用的是 [esbuild](https://esbuild.github.io/)
  esbuild 编译快，开发环境项目启动快
  - 后续可能被 Rolldown（ Rust 版本的 Rollup） 取代
- 生产环境打包使用的是 [rollup](https://rollup.nodejs.cn/)
  - 插件 API 与使用 esbuild 作为打包器并不兼容
  - Rollup 性能好和有更灵活的插件api

### 了解 vite 时引发了解其它知识

#### 构建工具和打包工具

构建工具？打包工具？ [参考](https://blog.csdn.net/qq_17335549/article/details/136113757)
打包工具一般比较单一，像 webpack 官方定义为模块打包工具，但实际上还有其它功能比如：测试、编译、优化、压缩、等等，又习惯称为构建工具（webpack 构建本地开发环境，功能多）。
打包工具主要分为两类：

- 基于任务的打包工具（grunt 和 gulp）
- 基于模块的打包工具（webpack、rollup、parcel、esbuild、browserify）

vite？是一种新型前端构建工具，开发环境编译使用 esbuild，生产环境编译 rollup）

#### grun t 和 gulp

都是一个基于任务的构建工具，通过配置任务列表，实现前端项目的自动化构建。
...
- rollup
  ...
  - Rollup 基于 ESM 打包（所以 vite 不支持 commonJs 和 AMD，要使用得安装插件），打包生成的文件更小。
...
1. Babel、Polyfill、core-js 区别：

- Babel：只转化新的语法，不负责实现新版本 js 中新增的 api。将 es6 转 es5
  通过配置 browserslist 需要兼容的浏览器版本，使用 Babel 将高级语法转化为 browserslist 中配置的浏览器版本可运行的 js 语法

- Polyfill：（垫片）实现新版本 js 中新增的 api。
  开发使用 es6 api，而低版本浏览器所支持 es3 api，那么需要实现一个该方法使其可用，这个方法就是垫片。

- core-js：它是 JavaScript 标准库的 polyfill（垫片/补丁） ，新功能的 es 'api' 转换为大部分现代浏览器都可以支持运行的一个'api'
  补丁包集合。 从 Babel 7.4.0 开始，@babel/polyfill 这个包已经被弃用，取而代之的是直接包含 core-js/stable。

**备注：**

- js 垫片：意思是在低级环境中用高级语法时，在低级环境中手动实现的高级功能，模拟高级环境。（低浏览器没有高浏览器语法，要自己实现或使用第三方插件来解决，使得可以使用高级浏览器语法）

## 大文件分片上传

- [vue3 + node 大文件分片上传(前后端, 包括文件秒传、断点续传)-CSDN 博客](https://blog.csdn.net/liyixuanstay/article/details/136660752)
